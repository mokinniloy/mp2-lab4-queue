...
void TSystem::printResults()
{
  ...
  std::cout << "Idle clocks: " << idleClocks << "\033[93m(" << (double)idleClocks / clockCount * 100 << "%)\033[0m" << std::endl;

  std::cout << "Task count: " << taskCount << std::endl;
  std::cout << "Completed tasks: " << completedTasks << std::endl;
  std::cout << "Skipped tasks: " << skippedTasks << "\033[93m(" << (double)skippedTasks / taskCount * 100 << "%)\033[0m" << std::endl;
